 ...
 *******************************************************************************
  ...
  函数列表   : 1) HAL_CAN_RxFifo0MsgPendingCallback() 【HAL库函数，CAN回调函数】
							 ...
							 5) Shoot_Motor3508() 									【外部调用：使用处】
							 6) Shoot_Motor3508_Disable() 					【外部调用：使用处】
...
/* 内部宏定义 ----------------------------------------------------------------*/
#define PIT_ANGLE(X) X>5000?(X-8491):(X-300)
/* 内部自定义数据类型的变量 --------------------------------------------------*/
...
void CAN_GET_Referee(uint8_t CAN_RX_date[]);
void CAN_SHOOT_interrup(uint8_t CAN_RX_date[]);
/* 函数主体部分 --------------------------------------------------------------*/
//===================================================================================================================//
/****************************************************CAN中断回调******************************************************/
//===================================================================================================================//
...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan)
{
 ...
 CAN_RxHeaderTypeDef  CAN1_Rx_Header;
	...
	if(hcan == &hcan1)
	{
		...
		{
			case CAN_DRIVERPLATE_3508MOTO_ID://拨盘电机
			{
 			Refresh_Device_OffLine_Time(DRIVERPLATE_3508MOTO_TOE);//刷新时间待加
				if(motor_get[DRIVERPLATE_3508MOTO].msg_cnt++ <= 50)	
				{
					Get_Moto_Offset(&motor_get[DRIVERPLATE_3508MOTO],CAN1_RX_date);
				}
				else
				{
					motor_get[DRIVERPLATE_3508MOTO].msg_cnt = 51;	
					Get_Moto_Measure_3508(&motor_get[DRIVERPLATE_3508MOTO],CAN1_RX_date);
				}
			}break;
			
			case CAN_TRIGGER_2006MOTOR_ID://拨弹电机
			{
 			Refresh_Device_OffLine_Time(TRIGGER_2006MOTOR_TOE);//刷新时间待加
			...
			case CAN_YAW_6020MOTOR_ID://yaw轴电机反馈
			{
 			Refresh_Device_OffLine_Time(YawGimbalMotor_TOE);//刷新时间待加
				if(motor_get[YAW_6020MOTOR].msg_cnt++ <= 50)
				{
					Get_Moto_Offset(&motor_get[YAW_6020MOTOR],CAN1_RX_date);
				}
				else
				{
					motor_get[YAW_6020MOTOR].msg_cnt = 51;
					Get_Moto_Measure_6020(&motor_get[YAW_6020MOTOR],CAN1_RX_date);
				}
			}break;
   case CAN_CHASSIS_ID://底盘板通信
			{
				CAN_GET_Referee(CAN1_RX_date);
			}break;
			case CAN_SHOOT_ID:
			{
			 CAN_SHOOT_interrup(CAN1_RX_date);
			}break;
			default: break;
		}
	}
	...
	else if(hcan == &hcan2)
	{
	  HAL_CAN_GetRxMessage(hcan, CAN_RX_FIFO0, &CAN2_Rx_Header, CAN2_RX_date);
		...
		{
			case CAN_SHOOTR_3508MOTOR_ID://摩擦右电机
			{
 				Refresh_Device_OffLine_Time(SHOOTR_3508MOTOR_TOE);//刷新时间待加
				
				if(motor_get[SHOOTR_3508MOTOR].msg_cnt++ <= 50)	
				{
					Get_Moto_Offset(&motor_get[SHOOTR_3508MOTOR],CAN2_RX_date);
				}
				else
				{	
					motor_get[SHOOTR_3508MOTOR].msg_cnt = 51;	
					Get_Moto_Measure_3508(&motor_get[SHOOTR_3508MOTOR],CAN2_RX_date);
				}
			}break;
			
			case CAN_SHOOTL_3508MOTOR_ID://摩擦左电机 
			{
				Refresh_Device_OffLine_Time(SHOOTL_3508MOTOR_TOE);//刷新时间待加
				if(motor_get[SHOOTL_3508MOTOR].msg_cnt++ <= 50)	
				{
					Get_Moto_Offset(&motor_get[SHOOTL_3508MOTOR],CAN2_RX_date);
				}
				else
				{
				motor_get[SHOOTL_3508MOTOR].msg_cnt=51;
				Get_Moto_Measure_3508(&motor_get[SHOOTL_3508MOTOR], CAN2_RX_date);
				}
			}break;			
			
			case CAN_PITCH_6623MOTOR_ID://pitch轴电机反馈
			{
 			Refresh_Device_OffLine_Time(PitchGimbalMotor_TOE);//刷新时间待加
				
				if(motor_get[PITCH_6623MOTOR].msg_cnt++ <= 50)
				{
					Get_Moto_Offset(&motor_get[PITCH_6623MOTOR],CAN2_RX_date);
				}
				else
				{
					motor_get[PITCH_6623MOTOR].msg_cnt = 51;
					Get_Moto_Measure_6623(&motor_get[PITCH_6623MOTOR],CAN2_RX_date);
				}
			}break;
   default: break;
		}
...
HAL_StatusTypeDef CAN_Send_RemoteDate( CAN_HandleTypeDef * hcan,
									                uint16_t key_v, int16_t rc_ch0, int16_t rc_ch1, uint8_t rc_s1, uint8_t rc_s2)
{
...
HAL_StatusTypeDef CAN_Send_MouseDate( CAN_HandleTypeDef * hcan,
									                int16_t x, int16_t y, int16_t pit, uint8_t press_l, uint8_t press_r)
{
	static CAN_TxHeaderTypeDef  CAN_Send_MouseDate;
	uint8_t CAN_TX_DATA[8];

	CAN_Send_MouseDate.DLC = 0x08;
	CAN_Send_MouseDate.IDE = CAN_ID_STD;
	CAN_Send_MouseDate.RTR = CAN_RTR_DATA;
	CAN_Send_MouseDate.StdId = 0x111;
	...
	CAN_TX_DATA[4] = pit >> 8;
	CAN_TX_DATA[5] = pit;
	CAN_TX_DATA[6] = press_l;
	CAN_TX_DATA[7] = press_r;

	if (HAL_CAN_AddTxMessage(hcan, &CAN_Send_MouseDate, CAN_TX_DATA, (uint32_t *)CAN_TX_MAILBOX0) == HAL_OK)		
		return HAL_OK;
	else 
		return HAL_ERROR;	
}
...
void Get_Moto_Measure_6623(motor_measure_t *ptr,uint8_t CAN_RX_date[])
{
	...
	ptr->pit_angle = PIT_ANGLE(ptr->angle);
...
void CAN_GET_Referee(uint8_t CAN_RX_date[])
{
	Referee.heat = (CAN_RX_date[0]<<8 | CAN_RX_date[1]);
	Referee.Max_speed = (CAN_RX_date[2]<<8 | CAN_RX_date[3]);
	bullet_speed = (CAN_RX_date[4]<<8 | CAN_RX_date[5]);
	Referee.bullet_speed = (bullet_speed/100.0f);
	//	plate.Bullets_out = (CAN_RX_date[4]<<8 | CAN_RX_date[5]);
}
